...
if Args['resolution']:
	...
	binCov=prf.iBinCoverage(Args['contacts'],c2s,Args['resolution'],out=out_name,chrm_index=l2i,pointview=Args['pointview'],log=Args['log_file'])
	elp = timeit.default_timer() - start_time
	...
	contactDistanceHash = prf.iDistanceRead(maxd,path=Args['contacts'],pointview=Args['pointview'],resolution=Args['resolution'],coverage=binCov,log=Args['log_file'])
	...
	prf.iTotalContactListing(meanHash,binCov,Args['resolution'],out_name,pointview=Args['pointview'],path=Args['contacts'],log=Args['log_file'])
	del meanHash
	del binCov
	elp = timeit.default_timer() - start_time
...
if Args['low_resolution']:
	...
	binCovLow=prf.iBinCoverage(Args['contacts'],c2s_low,Args['low_resolution'],out=out_name_low,chrm_index=l2i,pointview=Args['pointview'],log=Args['log_file'])
	elp = timeit.default_timer() - start_time
	prf.printlog('...bin coverage calculated for %.2fs' % elp, Args['log_file'])
