def markgenerate(MT,resolution,name,output_dir):
	...
	r,k = resolution//2,0
	for chrm in MT:
		for i in range(len(MT[chrm])):
			k += 1
			f1.write('%s\t%i\t%i\t%s\t%i\t%i\t%i\n' % (MT[chrm][i][0], MT[chrm][i][1]*resolution+r-50,MT[chrm][i][1]*resolution+r+50, chrm, i*resolution+r-50,i*resolution+r+50, k) )
			f2.write('%s\t%i\t%i\t%s\t%i\t%i\t%i\n' % (chrm, i*resolution+r-50,i*resolution+r+50, MT[chrm][i][0], MT[chrm][i][1]*resolution+r-50,MT[chrm][i][1]*resolution+r+50, k) )
	f1.close()
	f2.close()
